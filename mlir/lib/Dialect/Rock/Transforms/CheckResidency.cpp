...
public:
  LogicalResult checkFunc(gpu::BinaryOp bop, ROCDL::ROCDLKernelAttr &kernel) {
...
#define GET_INT_VALUE2(var, metadata, name)                                    \
  int64_t var = 0;                                                             \
  if (auto var##Attr = metadata.getMDAttr<IntegerAttr>(name))                  \
    var = var##Attr.getInt();                                                  \
  else                                                                         \
    return removeBinary()
    ...
    GET_INT_VALUE2(nextVGPR, kernel, "vgpr_count");
    GET_INT_VALUE2(nextSGPR, kernel, "sgpr_count");
      ...
      for (Block &block : region) {
        for (Operation &op : block) {
          ...
          for (Attribute objRaw : binOp.getObjects()) {
            ...
            if (DictionaryAttr props = obj.getProperties()) {
              metadata = props.getAs<ROCDL::ROCDLObjectMDAttr>(
                  ROCDL::getROCDLObjectMetadataName());
            }
            ...
            for (auto [name, attr] : metadata) {
              ROCDL::ROCDLKernelAttr kernel = attr;
            ...
            NamedAttrList props(obj.getProperties());
            props.set(ROCDL::getROCDLObjectMetadataName(),
                      ROCDL::ROCDLObjectMDAttr::get(
                          updatedMD.getDictionary(context)));
            objects.push_back(gpu::ObjectAttr::get(
                context, obj.getTarget(), obj.getFormat(), obj.getObject(),
                props.getDictionary(context)));
            changedObj = true;
          }
